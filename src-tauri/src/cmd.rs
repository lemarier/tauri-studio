...
pub fn generate_project(gist: Option<String>) -> TauriProject {
    let local_path = TempDir::new("tauri-builder")
    ...
    let tauri_conf_path = find_file_in_path(local_path.clone(), "**/tauri.conf.json")
        .expect("Unable to get tauri.conf.json")
        .unwrap();
    let main_html_file = find_file_in_path(local_path.clone(), "**/index.html")
        .expect("Unable to get index.html")
        .unwrap();
    let main_tauri_file = find_file_in_path(local_path.clone(), "**/main.rs")
        .expect("Unable to get main.rs")
        .unwrap();
    let main_cargo_file = find_file_in_path(local_path.clone(), "**/Cargo.toml")
        .expect("Unable to get cargo.toml")
        .unwrap();
